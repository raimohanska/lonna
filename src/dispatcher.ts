import { Callback, endEvent, Event, isEnd, Observer, Unsub, valueObserver } from "./abstractions";
...
function DispatchList() {
    ...
    function dispatch(index: number, value: any) {
        ...
        if (observers) for (const pair of observers) {
            if (removed.has(pair)) {
    ...
    function remove(pair: ObserverPair) {
        ...
        if (index >= 0) {
            ...
            _observers = [..._observers.slice(0, index), ..._observers.slice(index + 1)]
            return true
        }   
        return false
    }
