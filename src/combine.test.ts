...
describe("combine", () => {
  describe("Forms", () => {
    it("Function last", () => {
      ...
      expect(combine(constant(1), constant(2), (x, y) => x * y).get()).toEqual(
        2
      )
      expect(
        combine(
    ...
    it("Function first", () => {
      ...
      expect(combine((x, y) => x * y, constant(1), constant(2)).get()).toEqual(
        2
      )
    })
  ...
  it("Minimizes calls to mapping function", () => {
    ...
    const a = constant("a")
    const b = constant("b")
    const mapped = combine(a, b, (left, right) => {
      count++
    })
  ...
  it("toString", () =>
    expect(combine(constant(1), constant(2), () => {}).toString()).toEqual(
      "Property combine([constant(1),constant(2)],fn)"
    ))
})
...
describe("combineAsArray", () => {
  ...
  it("toString", () =>
    expect(combineAsArray([constant(1), constant(2)]).toString()).toEqual(
      "Property combineAsArray(constant(1),constant(2))"
    ))
})
