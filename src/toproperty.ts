  ...
  return (streamOrSubscribe: any) => {
    if (isEventStream(streamOrSubscribe)) {
      return new StatelessProperty(
        streamOrSubscribe.desc,
        get,
        mapSubscribe(streamOrSubscribe.subscribe.bind(streamOrSubscribe), get),
        streamOrSubscribe.getScope()
      )
    } else {
      return new StatelessProperty(
        [streamOrSubscribe, "toStatelessProperty", [get]],
        get,
        mapSubscribe(streamOrSubscribe, get),
        globalScope
      )
    }
  }
...
export function toPropertySeed<A>(
  property: Property<A> | PropertySeed<A>
): PropertySeed<A> {
  ...
  return new PropertySeedImpl<A>(
    property.desc,
    property.get.bind(property),
    property.onChange.bind(property)
  )
}
