...
export type EventStreamDelay<V> = (
  stream: EventStreamSeed<V>
) => EventStreamSeed<V>

export function transformChanges<A>(
  methodCallDesc: MethodDesc,
  transformer: EventStreamDelay<A>
): UnaryTransformOp<A>
export function transformChanges<A>(
  methodCallDesc: MethodDesc,
  transformer: EventStreamDelay<A>,
...
export function transformChanges<A, B>(
  methodCallDesc: MethodDesc,
  transformer: EventStreamDelay<A>,
  scope?: Scope
): any {
  return (x: any) => {
    ...
    if (isEventStreamSeed<A>(x)) {
      ...
      r = new AtomSeedImpl(
        ...
        () => source.get(),
        (onValue, onEnd) => {
          return transformer(changes(source))
            .consume()
            .subscribe(onValue, onEnd)
        },
        source.set
      )
    } else if (isPropertySeed<A>(x)) {
      const source = x.consume()
      r = new PropertySeedImpl(
        desc,
        () => source.get(),
        (onValue, onEnd) => {
          return transformer(changes(source))
