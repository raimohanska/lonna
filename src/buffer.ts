...
export function bufferWithTime<V>(
  delay: number | DelayFunction
): (src: EventStream<V> | EventStreamSeed<V>) => EventStreamSeed<V[]> {
  return (src) =>
...
export function bufferWithCount<V>(
  count: number
): (src: EventStream<V> | EventStreamSeed<V>) => EventStreamSeed<V[]> {
  return (src) =>
    bufferWithTimeOrCount(["bufferWithCount", [count]], src, undefined, count)
}

function bufferWithTimeOrCount<V>(
  desc: MethodDesc,
  src: EventStream<V> | EventStreamSeed<V>,
  delay?: number | DelayFunction,
  count?: number
): EventStreamSeed<V[]> {
...
function buffer<V>(
  desc: MethodDesc,
  src: EventStream<V> | EventStreamSeed<V>,
  onInput: BufferHandler<V> = nop,
  onFlush: BufferHandler<V> = nop
): EventStreamSeed<V[]> {
