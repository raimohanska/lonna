...
export type GenericObjectTemplate<
  T,
  O extends ObservableSeed<any, any, any>
> = {
  [K in keyof T]: T[K] extends ObservableSeed<infer I, any, any>
    ? T[K] extends O
      ? I
      : never
...
export type GenericArrayTemplate<
  T,
  O extends ObservableSeed<any, any, any>
> = Array<
  T extends ObservableSeed<infer I, any, any>
    ? T extends O
      ? I
      : never
...
export type GenericCombinedTemplate<
  T,
  O extends ObservableSeed<any, any, any>
> = T extends Record<any, any>
  ? GenericObjectTemplate<T, O>
  ...
  : T extends ObservableSeed<infer I2, any, any>
  ? T extends O
    ? I2
    : never
  : T
...
export function combineTemplate<T>(
  template: T
): Property<GenericObjectTemplate<T, Property<any>>> {
  if (!containsObservables(template)) return constant(template) as any
  const [observables, combinator] = processTemplate<T, Property<any>>(
    template,
    (x: ObservableSeed<any, any, any>) => {
  ...
  return rename(
    "combineTemplate(..)",
    map(combinator as any)(
      combineAsArray(observables as Property<any>[])
    ) as any
  )
}
...
export function combineTemplateS<T>(
  template: T,
  scope: Scope
): Property<GenericObjectTemplate<T, PropertySeed<any> | Property<any>>>
export function combineTemplateS<T>(
  template: T
): PropertySeed<GenericObjectTemplate<T, PropertySeed<any> | Property<any>>>

export function combineTemplateS<T>(
  template: T,
  scope?: Scope
): ObservableSeed<any, any, any> {
  if (!containsObservables(template)) return constant(template) as any

  const [observables, combinator] = processTemplate<T, PropertySeed<any>>(
    template,
    (x: ObservableSeed<any, any, any>) => {
...
function processTemplate<T, Prop>(
  template: T,
  mapObservable: (o: ObservableSeed<any, any, any>) => Prop
): [Prop[], Function] {
  function current(ctxStack: Ctx[]) {
  ...
  function compile(key: any, value: any) {
    if (isObservableSeed(value)) {
      ...
      const popContext = function (ctxStack: Ctx[]) {
        ctxStack.pop()
      }
...
function containsObservables(
  value: any,
  match: Predicate<any> = isObservableSeed
) {
  if (match(value)) {
    return true
  } else if (
