...
describe("EventStream.bufferWithTime", function () {
  ...
  describe("works with empty stream", () =>
    expectStreamEvents(() => bufferWithTime(1)(never()), []))
  ...
  it("toString", () =>
    expect(never().pipe(bufferWithTime(1)).toString()).toEqual(
      "EventStreamSeed never.bufferWithTime(1)"
    ))
})
...
describe("EventStream.bufferWithCount", function () {
  ...
  it("toString", () =>
    expect(bufferWithCount(1)(never()).toString()).toEqual(
      "EventStreamSeed never.bufferWithCount(1)"
    ))
})
