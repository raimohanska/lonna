...
describe("Bacon.fromEvent", function () {
  ...
  const bindUnbindSource = function () {
    return {
      cleaned: false,
  ...
  describe("eventSource is a string (the name of the event type to bind)", function () {
    ...
    it("should create EventStream from on/off event", function () {
      ...
      take(1)(fromEvent<string>(src, "test")).forEach((value) => {
        values.push(value)
      })() // TODO: currently Lonna streams don't auto-unsub on end. Should they?
    ...
    it("toString", () =>
      expect(fromEvent(onOffSource(), "click").toString()).toEqual(
        "EventStream fromEvent({cleaned:false,on:fn,off:fn},click)"
      ))
  })
  ...
  describe("eventSource is a function (a custom bind/unbind handler)", function () {
    describe("should create EventStream from DOM object", () =>
      expectStreamEvents(
        function () {
          ...
          const element = toEventTarget(emitter)
          return take(1)(
            fromEvent(element, (binder, listener) => binder("click", listener))
          )
        },
        ["x"]
      ))
    ...
    describe("should create EventStream from EventEmitter", () =>
      expectStreamEvents(
        function () {
          ...
          emitter.on("newListener", () => soon(() => emitter.emit("data", "x")))
          return take(1)(
            fromEvent(emitter, (binder, listener) => binder("data", listener))
          )
        },
        ["x"]
      ))
    ...
    it("should clean up event listeners from EventEmitter", function () {
      ...
      take(1)(
        fromEvent(emitter, (binder, listener) => binder("data", listener))
      ).forEach(function () {})() // TODO: currently Lonna streams don't auto-unsub on end. Should they?
      emitter.emit("data", "x")
      expect(emitter.listeners("data").length).toEqual(0)
    })
    ...
    it("should clean up event listeners from DOM object", function () {
      const emitter = new EventEmitter()
      const element = toEventTarget(emitter)
      const dispose = fromEvent(element, (binder, listener) =>
        binder("click", listener)
      ).forEach(function () {})
      dispose()
      expect(emitter.listeners("click").length).toEqual(0)
    })
    ...
    it("should create EventStream from on/off event", function () {
      ...
      const src = onOffSource()
      take(1)(
        fromEvent<string>(src, (binder, listener) => binder("test", listener))
      ).forEach((value) => {
        values.push(value)
      })() // TODO: currently Lonna streams don't auto-unsub on end. Should they?
      expect(values).toEqual(["test"])
    ...
    it("toString", () =>
      expect(
        fromEvent(onOffSource(), (binder, listener) =>
          binder("click", listener)
        ).toString()
