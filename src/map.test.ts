...
describe("EventStream.map", () => {
  describe("should map with given function", () =>
    expectStreamEvents(() => map(times2)(series(1, [1, 2, 3])), [2, 4, 6]))

  describe("can map to a Property value", () => {
    expectStreamEvents(() => map(constant(2))(series(1, [1, 2, 3])), [2, 2, 2])
  })
  it("toString", () => {
    expect(map(() => true)(never()).toString()).toEqual(
      "EventStream never.map(fn)"
    )
  })
})
